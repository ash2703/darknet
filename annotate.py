...
def generate_annotations(ii, return_dict):
    ...
    strID = dictLine['ID']
    img_path = '/data/obj/Images{}.jpg'.format(strID)
    if os.path.exists(img_path):  #annotation file has all the labels but we are only using 1/4th data
        ...
        with open('/data/obj/Images{}.txt'.format(strID), 'w+') as txtf:
            ...
            for label in dictLine['gtboxes']:
                ...
                # Person BB
                px = float(label['vbox'][0])
                py = float(label['vbox'][1])
                pw = float(label['vbox'][2])
                ph = float(label['vbox'][3])
...
#change path to '/content/drive/My Drive/darknet/annotation_val.odgt'
#for annotating the validation images
